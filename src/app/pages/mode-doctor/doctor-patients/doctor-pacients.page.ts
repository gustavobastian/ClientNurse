...
export class DoctorPacientsPage implements OnInit {
  ...
  async onClickNotes() {
    ...
    await this.MQTTServ.MQTTClientLocal.subscribe(responseNoteTopic).on(
      (Message) => {
    ...
    let b = new MessageModel(
      this.doctorName,
      this.patientLocal.id.toString(),
      0,
      5
    );
  ...
  async onClick() {
    ...
    await this.MQTTServ.MQTTClientLocal.subscribe(responseInfoTopic).on(
      (Message) => {
  ...
  handleDelete(event: KeyboardEvent, i: number) {
